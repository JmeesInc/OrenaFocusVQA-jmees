#!/usr/bin/env bash
# expQ00-A を **4×4090 DDP** で回す（機体側で実行）。
#   bash train_ddp_q00.sh [nproc] [追加引数（--train-limit 160 など）]
#
...
CFG="$REPO/workspace/expQ00_frame_alldata/config_Q00A_frame_alldata_ddp4.yaml"
...
stdbuf -oL -eL "$REPO/.venv/bin/python" -m torch.distributed.run \
  --nproc_per_node="$NP" --master_port=29541 --no-python \
