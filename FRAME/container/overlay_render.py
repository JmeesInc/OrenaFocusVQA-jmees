"""コンテナ側の重畳描画 — **`workspace/expM00_frame_overlay/overlay_render.py` の逐語コピー**。

Source: workspace/expM00_frame_overlay/overlay_render.py（2026-09-02 時点）
学習に使ったのと**同一のコード**であること自体が要件なので、**書き換えない**。
差し替えるときは元ファイルをコピーし直し、md5 を README に残す。
（[[port-inference-call-verbatim]]: 移植は呼び出しごと写す。1行の違いが条件を別物にする）

expM00-A の学習条件:
  variant = r1（クラス名は各クラス「1番」だけ + インスタンス番号）/ conf = 0.25 /
  検出器 = expF40(7cls) + expF39(clip specialist, 768x1344) の MergedDetector /
  描画は **モデルが見る幅（768px）に縮めてから**行う / 検出0件なら**重畳を作らない**
"""
...
def _load_decode_instances():
    ...
    p = Path(__file__).resolve().parent / "postprocess.py"
...
class MergedDetector:
    ...
    def predict(self, img_bgr: np.ndarray) -> list[dict]:
        ...
        dets = [d for d in self.all.predict(img_bgr, conf=lo)
                if d["cls"] not in self.spec]
