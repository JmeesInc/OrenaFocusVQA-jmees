...
def normalize(fmt: str, text: str) -> str:
    ...
    s = str(text).strip()
    # ★★v016: base から scratch で学習したモデルは **EOS を打たずに偽の user/assistant
    #   ターンを続ける**（expP00C の val 200 問で 181件=90.5%、expN02 で 84.8% 実測）。
    #   これを切らないと採点は全滅する（eval_N02_r64 の SCORE は **0.0026**）。
    #   FOCUS の正解は全形式1行なので、**最初の改行で切る**のは無条件に安全。
    #   生成側の stop_strings=["\n"] と二重の防御にする。
    s = s.split("\n")[0].strip()
    if not s:
        return s
