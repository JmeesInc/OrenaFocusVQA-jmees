...
def main() -> None:
    ...
    with (ROOT / f"workspace/fold/qa_{args.version}/qa_split.csv").open() as f:
        for r in csv.DictReader(f):
            if r["track"] == "SEGMENT" and int(r["fold"]) == args.fold \
    ...
    for dsn in ("heico", "lapchole"):
        ds = FocusDataset(dsn, DatasetSplit.ALL, Track.SEGMENT)
    ...
    (OUT / "batch.json").write_text(json.dumps(
        {"qIDs": [r.qID for r in picked], "n": len(picked), "track": "segment"}, indent=1))
