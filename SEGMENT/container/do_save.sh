...
# ★dl1 の docker は既定 runtime が nvidia でそれが壊れている
#   （`nvidia-container-runtime: executable file not found`）。この検算は GPU が要らないので
#   `--runtime=runc` を明示する。GPU が要るのは do_test_run.sh だけ（dl2 で回す）。
built_id="$(docker run --rm --runtime="${DOCKER_RUNTIME:-runc}" --entrypoint cat "$DOCKER_TAG" /opt/app/resources/.build_id)"
