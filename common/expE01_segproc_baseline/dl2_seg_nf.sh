...
TAG="eval_expE06g_seg${NF:-64}f${SIZE:-448}${TAGSUF:-}_n2000"
echo "=== $(date '+%F %T') INFER SEGMENT 64f ==="
"$PY" "$D/run_infer.py" --track SEGMENT --fold 0 --part val --limit 2000 \
  --n-frames ${NF:-64} --size ${SIZE:-448} --version v004 ${EXTRA:-} --adapter "$AD" --out-tag "$TAG"
echo "=== $(date '+%F %T') INFER rc=$? ==="
"$PY" "$D/eval_seg.py" "$R/$TAG" --track SEGMENT
echo "=== $(date '+%F %T') JUDGE rc=$? ==="
"$PY" "$D/compare_runs.py" "$R/${BASE:-eval_expE06g_seg64f448_n2000}" "$R/$TAG" --labels base ${NF:-64}f
echo "=== $(date '+%F %T') DONE ==="
